...
<style>
    ...
    .social-icon { transition: all .4s cubic-bezier(.4,0,.2,1); }
    .social-icon:hover { transform: translateY(-4px) scale(1.12) rotate(8deg); box-shadow: 0 10px 20px -5px rgb(249 115 22 / .3); }
    .social-icon svg { transition: all .4s ease; }
    ...
    <div class="max-w-7xl mx-auto bg-gray-900 rounded-[40px] md:rounded-[60px] p-10 md:p-14 relative overflow-hidden shadow-2xl border border-gray-800">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-12 relative z-10">
            ...
            <div class="md:col-span-4 space-y-6 text-center md:text-left">
                ...
                {{-- Icônes sociales (même charte que le footer visiteur) --}}
            ...
            <div class="md:col-span-3 text-center md:text-left">
                ...
                <ul class="space-y-4">
                    @foreach([['Accueil','/UserHome','home'], ['Communauté','/UserCommunity','users'], ['Mes recettes','/mes-recettes','chef-hat'], ['Mon profil','/profile','user'], ['Coups de cœur','/profile#coup-de-coeur','heart']] as $item)
                    <li>
                        <a href="{{ $item[1] }}" class="uf-link relative text-gray-400 hover:text-orange-500 text-sm transition-colors inline-flex items-center gap-2.5 font-medium group/nav">
                            <span class="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center text-orange-500 group-hover/nav:bg-orange-500 group-hover/nav:text-white transition-all">
                                <i data-lucide="{{ $item[2] }}" class="w-3.5 h-3.5"></i>
                            </span>
                            {{ $item[0] }}
                        </a>
                    </li>
